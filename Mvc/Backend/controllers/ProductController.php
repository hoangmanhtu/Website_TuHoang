...
// thêm sản phẩm mới
    public function create(){
        ...
        if(isset($_POST['submit'])){
            ...
            if(empty($this->error)){
              $total_product=$category_model->countProduct($category_id);
              $count_product=$total_product["total_product"]+1;
              $total_producer=$producer_model->countProducer($producer_id);
              $countProducer=$total_producer["total_product"]+1;
                $avatar='';
                ...
                if ($product_id) {
                  $category_model->count_product=$count_product;
                  $category_model->total_Product($category_id);
                  $producer_model->count_producer=$countProducer;
                  $producer_model->total_Producer($producer_id);
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($this->error)) {
              ...
              if($product['category_id'] == $category_id){
                $total_product=$category_model->countProduct($category_id);
                $count_product=$total_product["total_product"];
                $category_model->count_product=$count_product;
                $category_model->total_Product($category_id);
              }
              else{
                ...
                $total_product1=$category_model->countProduct($category_id1);
                $count_product1=$total_product1["total_product"]-1;
                $category_model->count_product=$count_product1;
                $category_model->total_Product($category_id1);
//
                $total_product=$category_model->countProduct($category_id);
                $count_product=$total_product["total_product"]+1;
                $category_model->count_product=$count_product;
              ...
              if($product['producer_id'] == $producer_id){
                $total_producer=$producer_model->countProducer($producer_id);
                $count_producer=$total_producer["total_product"];
                $producer_model->count_producer=$count_producer;
                $producer_model->total_Producer($producer_id);
              }
              else{

                $producer_id1=$product['producer_id'];
                $total_producer1=$producer_model->countProducer($producer_id1);
                $count_producer1=$total_producer1["total_product"]-1;
                $producer_model->count_producer=$count_producer1;
                $producer_model->total_Producer($producer_id1);
//
                $total_producer=$producer_model->countProducer($producer_id);
                $count_producer=$total_producer["total_product"]+1;
                $producer_model->count_producer=$count_producer;
                $producer_model->total_Producer($producer_id);
              }
