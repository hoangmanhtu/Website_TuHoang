...
class ShoppingCart extends Model {
  ...
  public function countTotalStaff($id)
  {
    $sql_count="select count(orders.id) from orders where admin_id=$id";
    $obj_select = $this->connection->prepare($sql_count);
    $obj_select->execute();
    return $obj_select->fetchColumn();
  }
  ...
  public function listOrderStaff($id,$pageSize,$page){
    $from=($page-1) * $pageSize;
    $obj_select=$this->connection->prepare("select * from orders where admin_id=$id
                    order by orders.created_at desc limit $from,$pageSize");
    $arr_select = [];
    $obj_select->execute($arr_select);
    $orders = $obj_select->fetchAll(PDO::FETCH_ASSOC);
  ...
  public function orderSearchStaff($id,$search,$pageSize,$page)
  {
    $from=($page-1) * $pageSize;
    $obj_select=$this->connection->prepare
    ("select * from orders where  admin_id=$id and ( phone like '%$search%' or fullname like '%$search%' )
                order by created_at desc limit $from,$pageSize");
    $obj_select->execute();
    $orders = $obj_select->fetchAll(PDO::FETCH_ASSOC);
    return $orders;
  }
  public function countorderSearchSatff($id,$search)
  {
    $obj_select=$this->connection->
    prepare("select count(id) from orders where  admin_id=$id and (phone like '%$search%' or fullname like '%$search%')");
