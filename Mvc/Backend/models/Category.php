...
//    chỉnh sửa danh mục
    ...
    public function countProduct($category_id){
      $sql_count="select total_product from categories where id=$category_id";
      $obj_count=$this->connection->prepare($sql_count);
      $obj_count->execute();
      $count=$obj_count->fetch(PDO::FETCH_ASSOC);
      return $count;
    }
    public function total_Product($category_id){
      $sql_update="update categories set `total_product`=:total_product where id=$category_id";
      $obj_update=$this->connection->prepare($sql_update);
      $arr_update=[
          ':total_product'=> $this->count_product,
      ];
      return $obj_update->execute($arr_update);
    }
}